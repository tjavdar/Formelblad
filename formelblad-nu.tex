...
\begin{itemize}
  \item $f\!:\Rone^n\to\Rone$ is convex if
    $
    f[(1-\lambda)\bdx^{(1)}+\lambda\bdx^{(2)}]
    \le
    (1-\lambda) f(\bdx^{(1)})+\lambda f(\bdx^{(2)})
    $.
    Jensen's inequality generalises for any convex linear combination of points:
    $
    f(\sum_{i=1}^p\lambda_i\bdx^{(i)})
    \le
    \sum_{i=1}^p\lambda_i f(\bdx^{(i)}).
    $

  \item If $f_{1,2}$ are convex and $\alpha_{1,2}\ge0$, then $\alpha_1f_1+\alpha_2 f_2$
  ...
  \item If $f\!:\Rone^n\to\Rone\in C^2$ is locally convex, then the
   ...
   \item  $f\!:\Rone^n\to\Rone$ is \textbf{concave} if $(-f)$ is convex, i.e.,
    $
    f[(1-\lambda)\bdx^{(1)}+\lambda\bdx^{(2)}]
    \ge
    (1-\lambda) f(\bdx^{(1)})+\lambda f(\bdx^{(2)})

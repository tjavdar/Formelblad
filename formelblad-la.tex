...
\subsection*{\Tr{Trigonometry}{Trigonometri}} %<<<
...
\subsection*{\Tr{The Unifying Theorem: Equivalent claims for a square matrix}%<<<
                {Huvudsatsen: Ekvivalents påståenden för en kvadratisk matris}
               $A_{n\times n}$}

  \begin{itemize}\addtolength{\itemsep}{-5pt}
  \item $A\sim I$ ($\mbox{rank}(A)=n$)
  \item $\det(A)\ne0$
  \item \Tr{A unique inverse}{Existerar entydig invers} \/ $A^{-1}$ \Tr{exists}{}
  \item
    \Tr{The system}{Ekvationssystemet}
    $A\bdx=\bdzero$
    \Tr{has only the trivial solution}{har endast den triviala lösningen}
    $\bdx=\bdzero$
  \item
    \Tr{The homogeneous system}{Ekvationssystemet}
    $A\bdx=\bdb$
    \Tr{has a unique solution for any}{har en entydig lösning för varje val av högerledet}
    $\bdb$
  \item $A$
    \Tr{'s rows are linear independent and form a basis for}{:s rader är linjärt oberoende och utgör en bas för}
    $\mathbb{R}^n$
  \item $A$
    \Tr{'s columns are linear independent and form a basis for}{:s kolonner är linjärt oberoende och utgör en bas för}
    $\mathbb{R}^n$
  \item
    \Tr{The linear map}{Avbildningen}
    $F(\bdx)=A\bdx$
    \Tr{is invertible}{är omvändbar}
  \item
    \Tr{All eigenvalues of}{Alla egenvärden till}
    $A$
    \Tr{are different from}{är skilda från}
    $0$.
  \end{itemize}%>>>
